...
class BaseScorer:
    ...
    def score(self, X, y):
        ...
        if self.verbose > 0:
            iterator = tqdm.tqdm(iterator, total=X.shape[1] * (X.shape[1] - 1) / 2, desc=f"Scoring {self.__class__.__name__}")

        for x1, x2 in iterator:
            self.X_pairings[(x1, x2)] = self._X_score(x1, x2)

        # Assumption: y_names and y will never be scored together
        iterator = itertools.product(range(X.shape[1]), range(max(len(self.y_names), 1)))
        if self.verbose > 0:
            iterator = tqdm.tqdm(iterator, total=X.shape[1] * max(len(self.y_names), 1), desc=f"Scoring {self.__class__.__name__}")
        for x, y in iterator:
    ...
    def load_cache(self):
        ...
        with open(os.path.join(self.cache), "rb") as f:
            dat = pickle.load(f)
            if self.X_n != dat["X_n"]:
                raise ValueError(f"Feature count does not match cache {self.X.shape[1]} != {dat['X_n']}")
            if self.y_n != dat["y_n"]:
                raise ValueError(f"Label count does not match cache {self.y_n} != {dat['y_n']}")
...
class LinearScorer(BaseScorer):
    ...
    def _X_score(self, x1, x2):
        ...
        return sum([self.alpha[i] * self.scorers[i].X_score(x1, x2) for i in range(len(self.scorers))])

    def _X_y_score(self, x, y):
        # Linear combination of the alpha weights and the results from the individual scorers.
        return sum([self.alpha[i] * self.scorers[i].X_y_score(x, y) for i in range(len(self.scorers))])
    ...
    def __init__(self, X, y, X_names=None, y_names=None, cache=None, **kwargs):
        ...
        if self.sts_function is None:
            ...
            if len(inspect.signature(self.sts_function).parameters) != 2:
                raise ValueError(f"sts_function {self.sts_function} should accept two parameters")
...
class SentenceTransformerScorer(BaseSTSScorer):
    def __init__(self, X, y, X_names=None, y_names=None, cache=None, model_path=None,
                 verbose=0, **kwargs):
        if model_path is None:
            raise ValueError("model_path must be specified.")
        self.model_path = model_path

        super().__init__(X, y, X_names=X_names, y_names=y_names, cache=cache,
                         sts_function=lambda a, b: None, verbose=verbose, **kwargs)

    def score(self, X, y):
        if self.sts_function("", "") is None:
            from sentence_transformers import SentenceTransformer, util
            self.model = SentenceTransformer(self.model_path, device="cuda")
            self.sts_function = lambda x1, x2: \
                util.pytorch_cos_sim(self.model.encode(x1), self.model.encode(x2)).item()
        return super().score(X, y)


class GensimScorer(BaseSTSScorer):
    def __init__(self, X, y, X_names=None, y_names=None, cache=None, model_path=None,
                 verbose=0, model_type: type=None, **kwargs):
        if model_path is None:
            raise ValueError("model_path must be specified.")
        self.model_path = model_path
