...
from sts_select.scoring import (BaseSTSScorer, GensimScorer, LinearScorer,
                                MIScorer, SentenceTransformerScorer, PearsonsRScorer, Chi2Scorer, FScorer)
...
def compute_similarities(
    ...
    scorers[("PearsonsRScorer", None, None)] = PearsonsRScorer(
        X,
        y_truth,
        random_state=config["seed"],
        cache=config["path"]["pearson_r_cache"],
        verbose=config["verbose"],
    )
    scorers[("FScorer", None, None)] = FScorer(
        X,
        y_truth,
        random_state=config["seed"],
        cache=config["path"]["f_score_cache"],
        verbose=config["verbose"],
    )
    ...
    if config["data"]["name"] == "ppsp":
        ...
        for scorer_name in ["MIScorer", "PearsonsRScorer", "FScorer"]:

            linear_scorer = LinearScorer(
                X,
                y_truth,
                scorers=[scorers[(scorer_name, None, None)], scorers[("STSScorer", "ClinicalBERT", "ext")]],
                alpha=[1, 1],
                verbose=config["verbose"],
            )
            scorers[(f"LinearScorer-{scorer_name}", "ClinicalBERT", "ext")] = linear_scorer
    ...
    for dset, model in itertools.product(dset_options, model_options):
        ...
        for scorer_name in ["MIScorer", "PearsonsRScorer", "FScorer"]:
            linear_scorer = LinearScorer(
                X,
                y_truth,
                scorers=[scorers[(scorer_name, None, None)], sts_scorer],
                alpha=[1, 1],
                verbose=config["verbose"],
            )
            scorers[(f"LinearScorer-{scorer_name}", model, dset)] = linear_scorer
