...
from gensim import SkipgramModel, FastTextModel

from data import CLINICAL_VOCAB_NAME, GEN_VOCAB_NAME, COMBINED_VOCAB_NAME
...
def set_env_vars():
    ...
    os.environ["TRANSFORMERS_CACHE"] = os.path.join(os.path.dirname(__file__), "transformers")
...
if __name__ == "__main__":
    ...
    for dset_name, model_name in itertools.product(dset_options, model_options):
        ...
        if os.path.exists(save_location) and not args.overwrite:
            print(f"Model {model_name} already exists for dataset {dset_name}. Skipping.")
            continue

        # Load the dataset
        print(stage_format.format(f"Loading Dataset {dset_name}"))

        train_data = ds.load_from_disk(os.path.join(os.path.dirname(__file__), dset_name))
        ...
        if "other" not in model_name:
            ...
            mapper = lambda x: InputExample(texts=[x["sent1"], x["sent2"]], label=x["score"])
            train_data_mapped = [mapper(x) for x in train_data]
            train_data_mapped = SentencesDataset(train_data_mapped, model=model)
            train_dataloader = DataLoader(train_data_mapped, shuffle=True, batch_size=args.batch_size)
            ...
            train_dataloader = [x["sent1"] for x in train_data] + [x["sent2"] for x in train_data]
            train_loss = None # Not used
            train_data_mapped = None # Not used

        model.fit(train_objectives=[(train_dataloader, train_loss)], epochs=args.epochs, warmup_steps=100)

        if "other" not in model_name:
            # Evaluate the final loss on the training set
            print(stage_format.format("Evaluating Model"))
            # Evaluator
            evaluator = EmbeddingSimilarityEvaluator.from_input_examples(train_data_mapped,
        ...
        losses_results.append([{"Dataset": dset_name, "Model": model_name, "Loss": eval_loss}])
